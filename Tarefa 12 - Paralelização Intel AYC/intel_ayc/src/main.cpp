/*!
 * \file main.cpp
 * \brief This file contains source code that solves the Work Hard - Play Hard
 * problem for the Acceler8 contest
 */
#include <cstdlib>
#include <fstream>
#include <iostream>
 ...
 * This structure don't need to be modified but feel free to change it if you
 * want.
 */
struct Parameters {
  string from;                /*!< The city where the travel begins */
  string to;                  /*!< The city where the conference takes place */
  unsigned long dep_time_min; /*!< The minimum departure time for the conference
                                 (epoch). No flight towards the conference's
                                 city must be scheduled before this time. */
  unsigned long dep_time_max; /*!< The maximum departure time for the conference
                                 (epoch). No flight towards the conference's
                                 city must be scheduled after this time.  */
  unsigned long ar_time_min;  /*!< The minimum arrival time after the conference
                                 (epoch). No flight from the conference's city
                                 must be scheduled before this time.  */
  unsigned long ar_time_max;  /*!< The maximum arrival time after the conference
                                 (epoch). No flight from the conference's city
                                 must be scheduled after this time.  */
  unsigned long
      max_layover_time; /*!< You don't want to wait more than this amount of
                           time at the airport between 2 flights (in seconds) */
  unsigned long
      vacation_time_min; /*!< Your minimum vacation time (in seconds). You can't
                            be in a plane during this time. */
  unsigned long
      vacation_time_max; /*!< Your maximum vacation time (in seconds). You can't
                            be in a plane during this time. */
  list<string>
      airports_of_interest; /*!< The list of cities you are interested in. */
  string flights_file;      /*!< The name of the file containing the flights. */
  string alliances_file;    /*!< The name of the file containing the company
                               alliances. */
...
void fill_travel(vector<Travel> &travels, vector<Flight> &flights,
                 string starting_point, unsigned long t_min,
                 unsigned long t_max);
...
/**
 * \fn Travel work_hard(vector<Flight>& flights, Parameters& parameters,
 * vector<vector<string> >& alliances) \brief Solve the "Work Hard" problem.
 * This problem can be considered as the easy one. The goal is to find the
 * cheapest way to join a point B from a point A regarding some parameters.
...
Travel work_hard(vector<Flight> &flights, Parameters &parameters,
  ...
  // First, we need to create as much travels as it as the number of flights
  // that take off from the first city
...
/**
 * \fn void compute_path(vector<Flight>& flights, string to, vector<Travel>&
 * travels, unsigned long t_min, unsigned long t_max, Parameters parameters)
 * \brief Computes a path from a point A to a point B. The flights must be
 * scheduled between t_min and t_max. It is also important to take the layover
 * in consideration. You should try to improve and parallelize this function. A
 * lot of stuff can probably done here. You can do almost what you want but the
 * program output must not be modified. \param flights All the flights that are
 * available. \param to The destination. \param travels The list of possible
 * travels that we are building. \param t_min You must not be in a plane before
 * this value (epoch) \param t_max You must not be in a plane after this value
 * (epoch) \param parameters The program parameters
 */
void compute_path(vector<Flight> &flights, string to, vector<Travel> &travels,
                  unsigned long t_min, unsigned long t_max,
                  Parameters parameters) {
...
/**
 * \fn Travel find_cheapest(vector<Travel>& travels, vector<vector<string>
 * >&alliances) \brief Finds the cheapest travel amongst the travels's vector.
...
/**
 * \fn void fill_travel(vector<Travel>& travels, vector<Flight>& flights, string
 * starting_point, unsigned long t_min, unsigned long t_max) \brief Fills the
 * travels's vector with flights that take off from the starting_point. This
 * function might probably be improved. \param travels A vector of travels under
 * construction \param flights All the flights that are available. \param
 * starting_point The starting point. \param travels The list of possible
 * travels that we are building. \param t_min You must not be in a plane before
 * this value (epoch). \param t_max You must not be in a plane after this value
 * (epoch).
 */
void fill_travel(vector<Travel> &travels, vector<Flight> &flights,
  ...
  for (unsigned int i = 0; i < flights.size(); i++) {
    if (flights[i].from == starting_point &&
        flights[i].take_off_time >= t_min && flights[i].land_time <= t_max) {
      Travel t;
...
/**
 * \fn time_t convert_to_timestamp(int day, int month, int year, int hour, int
 * minute, int seconde) \brief Convert a date to timestamp Parameter's names are
 * self-sufficient. You shouldn't modify this part of the code unless you know
 * what you are doing. \return a timestamp (epoch) corresponding to the given
 * parameters.
 */
time_t convert_to_timestamp(int day, int month, int year, int hour, int minute,
                            int seconde) {
 ...
 * \param s A string that represents a date with the following format
 * MMDDYYYYhhmmss with M = Month number D = Day number Y = Year number h = hour
 * number m = minute number s = second number You shouldn't modify this part of
 * the code unless you know what you are doing. \return a timestamp (epoch)
 ...
 * \brief This function split a string into a vector of strings regarding the
 * separator. \param result The vector of separated strings \param line The line
 * that must be split. \param separator The separator character.
 */
void split_string(vector<string> &result, string line, char separator) {
 ...
 * same line are in the same alliance. \param filename The name of the file
 * containing the alliances description.
 */
void parse_alliances(vector<vector<string>> &alliances, string filename) {
  string line = "";
  ifstream file;
...
/**
 * \fn bool company_are_in_a_common_alliance(const string& c1, const string& c2,
 * vector<vector<string> >& alliances) \brief Check if 2 companies are in the
 * same alliance. \param c1 The first company's name. \param c2 The second
 * company's name. \param alliances A 2D vector representing the alliances.
 * Companies on the same line are in the same alliance.
 */
bool company_are_in_a_common_alliance(const string &c1, const string &c2,
                                      vector<vector<string>> &alliances) {
  bool result = false;
...
/**
 * \fn bool has_just_traveled_with_company(vector<Flight>& flights_before,
 * Flight& current_flight) \brief The 2 last flights are with the same company.
...
/**
 * \fn bool has_just_traveled_with_alliance(Flight& flight_before, Flight&
 * current_flight, vector<vector<string> >& alliances) \brief The 2 last flights
 * are with the same alliance. \param flight_before The first flight. \param
 * current_flight The second flight. \param alliances The alliances. \return The
 * 2 flights are with the same alliance.
 */
bool has_just_traveled_with_alliance(Flight &flight_before,
                                     Flight &current_flight,
                                     vector<vector<string>> &alliances) {
 ...
 * \brief Indicates if the city has already been visited in the travel. This
 * function is used to avoid stupid loops. \param travel The travels. \apram
 * city The city. \return The current travel has never visited the given city.
 */
bool nerver_traveled_to(Travel travel, string city) {
  for (unsigned int i = 0; i < travel.flights.size(); i++)
...
/**
 * \fn void output_work_hard(vector<Flight>& flights, Parameters& parameters,
 * vector<vector<string> >& alliances) \brief Display the solution of the "Work
 * Hard" problem by solving it first. \param flights The list of available
 * flights. \param parameters The parameters. \param alliances The alliances
 * between companies.
 */
void output_work_hard(vector<Flight> &flights, Parameters &parameters,
                      vector<vector<string>> &alliances) {
  ...
  output << "“Work Hard” Proposition :"<<en  d l;
...
//./run -from Paris -to Los\ Angeles -departure_time_min 11152012000000
//-departure_time_max 11172012000000 -arrival_time_min 11222012000000
//-arrival_time_max 11252012000000 -max_layover 100000 -vacation_time_min 432000
//-vacation_time_max 604800 -vacation_airports Rio London Chicago -flights
// flights.txt -alliances alliances.txt
